...
fn prims(num: i32, edges: &[Vec<i32>], start: i32) -> usize {
    ...
    let mut pq = BinaryHeap::new(); // priority queue that stores (start node, end node, edge cost), sorted based on min edge cost
    ...
    // here we insert values into adjacency list g in the format (end node, edge cost)
    for edge in edges {
    ...
    let m = n - 1;
    let (mut edgecount, mut mstcost) = (0, 0); //(count of mst edges, total sum of mst edges)
    let mut mstedges: Vec<usize> = vec![0; m]; // to store the final edges of mst
